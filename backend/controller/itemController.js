...
import { getAllItems } from "../dbo_Services/getAllitemsService.js";
dotenv.config();
...
export const addItem = async (req, res) => {
  try {
    const { name, price, description, image, quantity, category, discount } =
      req.body;
...
export const showItem = async (req, res) => {
  try {
    ...
    console.log("filter", typeof filters);

    const cacheKey = `items_page_${page}_limit_${limit}_keyword_${keyword}_filter_${req.query.filter}`;
    ...
    // with SP
    const data = await getAllItems(limit, startIndex, keyword, filters);
    console.log(data, " data from homr controlers ");
...
export const updateItem = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, price, image, quantity, category, discount } =
      req.body;

    const reqItem = await pool.query("SELECT * FROM items WHERE item_id = $1", [
      id,
    ]);
...
//     console.log(checked + radio + "check and radio");
  ...
  try {
    ...
    cart.map((i) => {
      total +=
        ...
        (i.price * i.quantityInCart * i.discount) / 100;
    });
